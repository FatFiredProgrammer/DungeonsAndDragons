...
namespace DungeonsAndDragons.Game.Weapons
{
    ...
    /// </summary>
    public sealed class Maul : Weapon
    {
        /// <inheritdoc />
        public override string Name { get; } = nameof(Maul);

        /// <inheritdoc />
        /// <remarks>Note that we use Math.Max to make sure our damage is not negative.</remarks>
        public override int CalculateDamage(Character character) => Math.Max(0, GetBaseDamage(character) + Dice.Die6.Roll() + Dice.Die6.Roll());
    }
}
