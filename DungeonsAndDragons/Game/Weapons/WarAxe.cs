...
namespace DungeonsAndDragons.Game.Weapons
{
    ...
    /// </summary>
    public sealed class WarAxe : Weapon
    {
        /// <inheritdoc />
        public override string Name { get; } = nameof(WarAxe);

        /// <inheritdoc />
        /// <remarks>Note that we use Math.Max to make sure our damage is not negative.</remarks>
        public override int CalculateDamage(Character character) => Math.Max(1, GetBaseDamage(character) + Dice.Die8.Roll());
    }
}
